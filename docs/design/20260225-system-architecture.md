...
- S3互換Storage（東京リージョン優先、実装はS3StorageAdapter。R2は将来選択肢）
...
Phase 0 は `S3StorageAdapter` を標準実装とし、Storageリージョンを東京で固定する。  
`R2StorageAdapter` は将来の切替先として維持する。
...
ジョブ実行制御は `retry=3`、`timeout=300s` を標準値とする。
...
## 入力制約 / Upload Constraints

- 受け入れ形式: `image/jpeg`, `image/png`, `application/pdf`
- サイズ上限: 1ファイル 20MB
- 上限枚数: 1物件あたり最大20枚（UI/業務ルールで制御）
...
MVPはNext.js + Supabase + Cloud Run Workerの非同期構成とし、OCR/LLMはProvider抽象化を通じて実行する。東京リージョン統一、OpenAPI契約、ジョブ制御・監視基準を前提に実装判断を固定する。
