...
- `docs/adr/0002-cloud-tasks-for-job-dispatch.md` — Cloud Tasks をジョブディスパッチに採用
...
| Phase 0 Cloud Run デプロイ + ジョブディスパッチ | `docs/adr/0002-cloud-tasks-for-job-dispatch.md` | — | `apps/worker/`, `apps/web/lib/dispatch/`, `.github/workflows/deploy-worker.yml` |
...
- `POST /api/documents` 後に Cloud Tasks 経由で Worker（Cloud Run Service）へ自動ディスパッチできる。
...
| `#43` | Cloud Run デプロイパイプライン + ジョブディスパッチ連携 | `apps/worker/`, `apps/web/lib/dispatch/`, `.github/workflows/` |
